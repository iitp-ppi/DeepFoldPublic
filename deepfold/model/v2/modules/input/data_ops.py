...
from deepfold.model.v2.modules.geometry import Rigid, Rigid3Array, Rot3Array, Rotation, Vec3Array
...
def block_delete_msa(protein, config):
    ...
    block_num_seq = torch.floor(
        torch.tensor(num_seq, dtype=torch.float32, device=protein["msa"].device) * config.msa_fraction_per_block
    ).to(torch.int32)
...
def atom37_to_frames(protein, eps=1e-8):
    ...
    if is_multimer:
        ...
        gt_rotation = Rot3Array.from_two_vectors(origin - point_on_neg_x_axis, point_on_xy_plane - origin)
