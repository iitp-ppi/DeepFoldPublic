...
def is_homomer_or_monomer(chains: Iterable[FeatureDict]) -> bool:
    """Check whether homomer or monomer example."""

    return (
        len(np.unique(np.concatenate([np.unique(chain["entity_id"][chain["entity_id"] > 0]) for chain in chains]))) == 1
    )


def pair_and_merge(all_chain_features: Dict[str, FeatureDict]) -> FeatureDict:
    """Pair and merge features."""

    process_umerged_features_(all_chain_features)

    np_chains_list = list(all_chain_features.values())

    pair_msa_sequences = not is_homomer_or_monomer(np_chains_list)

    if pair_msa_sequences:
        np_chains_list = msa_pairing.create_paired_features(np_chains_list)
...
def process_umerged_features_(all_chain_features: Dict[str, FeatureDict]) -> None:
    ...
    for chain_features in all_chain_features.values():
        ...
        if "all_atom_positions" not in chain_features:
            ...
            chain_features["all_atom_positions"] = np.zeros(tuple(all_atom_mask.shape) + (3,))
...
def crop_chains(
    ...
    for chain in chains_list:
        cropped_chain = crop_single_chain(
...
def crop_single_chain(
    ...
    if pair_msa_sequences:
        ...
        # Reduce the number of un-paired sequences, by the number of times a sequence
        # from this chain's MSA is included in the paired MSA.
        # This keeps the MSA size for each chain roughly constant.
        ...
        # Restrict the unpaired crop size so that paired+unpaired sequences
        # do not exceed msa_seqs_per_chain for each chain.
...
def process_final(np_example: FeatureDict) -> FeatureDict:
    ...
    np_example = make_seq_mask(np_example)
    np_example = make_msa_mask(np_example)
    np_example = filter_features(np_example)

    return np_example


def correct_msa_restypes(np_example: FeatureDict) -> FeatureDict:
...
def make_seq_mask(np_example: FeatureDict) -> FeatureDict:
    np_example["seq_mask"] = (np_example["entity_id"] > 0).astype(np.float32)
    return np_example


def make_msa_mask(np_example: FeatureDict) -> FeatureDict:
...
def filter_features(np_example: FeatureDict) -> FeatureDict:
