...
from deepfold.model.v2.data import data_pipeline, feature_pipeline, mmcif_parsing, templates
from deepfold.utils.tensor_utils import dict_multimap, tensor_tree_map
...
class OpenFoldDataset(torch.utils.data.Dataset):
    ...
    def deterministic_train_filter(
        ...
        return all(
            [
                resolution_filter(resolution=resolution, max_resolution=max_resolution),
                aa_count_filter(seqs=seqs, max_single_aa_prop=max_single_aa_prop),
            ]
        )

    @staticmethod
    def get_stochastic_train_filter_prob(cache_entry: Any, *args, **kwargs) -> float:
        # Stochastic filters
        probabilities = []
...
class OpenFoldMultimerDataset(OpenFoldDataset):
    ...
    def __init__(
        ...
        super(OpenFoldMultimerDataset, self).__init__(
            datasets=datasets,
    ...
    @staticmethod
    def get_stochastic_train_filter_prob(cache_entry: Any, *args, **kwargs) -> list:
        # Stochastic filters
    ...
    def looped_samples(self, dataset_idx):
        ...
        while True:
            ...
            for _ in range(max_cache_len):
                ...
                if not self.deterministic_train_filter(
                    cache_entry=mmcif_data_cache_entry, is_distillation=is_distillation
                ):
                    continue
                ...
                weights.extend([[1.0 - p, p] for p in chain_probs])
...
class OpenFoldDataLoader(torch.utils.data.DataLoader):
    ...
    def _add_batch_properties(self, batch):
        ...
        samples = torch.multinomial(
            self.prop_probs_tensor, num_samples=1, replacement=True, generator=self.generator  # 1 per row
        )
...
class OpenFoldMultimerDataModule(OpenFoldDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(OpenFoldMultimerDataModule, self).__init__(
            config, template_mmcif_dir, max_template_date, train_data_dir, **kwargs
        )
