...
from deepfold.common import protein, residue_constants
from deepfold.losses.violation import compute_violation_metrics_np, find_structural_violations_np
from deepfold.relax import cleanup, utils
            ...
            for cat in cl_res.atoms():
                if cat.name == rat.name:
                    if not np.array_equal(cl_xyz[cat.index], ref_xyz[rat.index]):
                        raise ValueError(f"Coordinates of cleaned atom {cat} do not match " f"coordinates of reference atom {rat}.")
...
def make_atom14_positions(prot):
    ...
    residx_atom14_gt_mask = residx_atom14_mask * np.take_along_axis(prot["all_atom_mask"], residx_atom14_to_atom37, axis=1).astype(np.float32)
