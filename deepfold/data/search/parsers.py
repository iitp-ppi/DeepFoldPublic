    ...
    Returns:
        A tuple of:
            ...
            * The deletion matrix for the alignment as a list of lists.
                The element at `deletion_matrix[i][j]` is the number of residues
...
def parse_a3m(a3m_string: str) -> Tuple[Sequence[str], DeletionMatrix]:
    ...
    Args:
        a3m_string: The string contents of a a3m file.
            The first sequence in the file should be the query sequence.
...
def convert_stockholm_to_a3m(
    ...
    for seqname, sto_sequence in sequences.items():
        ...
        if remove_first_row_gaps:
            out_sequence = "".join(_convert_sto_seq_to_a3m(query_non_gaps, sto_sequence))
...
def _update_hhr_residue_indices_list(sequence: str, start_index: int, indices_list: List[int]) -> None:
...
def _parse_hhr_hit(detailed_lines: Sequence[str]) -> TemplateHit:
    ...
    Args:
        detailed_lines: A list of lines from a single comparison section
            between 2 sequences (which each have their own HMM's)
...
def parse_e_values_from_tblout(tblout: str) -> Dict[str, float]:
    ...
    # As per http://eddylab.org/software/hmmer/Userguide.pdf fields are space-delimited.
    # Relevant fields are (1) target name and (5) E-value (full sequence) (numbering from 1).
    for line in lines:
        fields = line.split()
        e_value = fields[4]
...
def parse_hmmsearch_sto(
    ...
    a3m_string = convert_stockholm_to_a3m(sto_string, remove_first_row_gaps=False)
    template_hits = parse_hmmsearch_a3m(query_sequence=query_sequence, a3m_string=a3m_string, skip_first=False)
