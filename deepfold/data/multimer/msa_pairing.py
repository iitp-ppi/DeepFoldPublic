...
def _make_msa_df(chain_features: Mapping[str, np.ndarray]) -> pd.DataFrame:
    ...
    msa_df = pd.DataFrame(
        {
            "msa_identifiers": chain_features["msa_identifiers_all_seq"],
            "msa_row": np.arange(len(chain_features["msa_identifiers_all_seq"])),
...
def _create_species_dict(msa_df: pd.DataFrame) -> Dict[bytes, pd.DataFrame]:
    ...
    species_lookup = {}
    for species, species_df in msa_df.groupby("msa_identifiers"):
