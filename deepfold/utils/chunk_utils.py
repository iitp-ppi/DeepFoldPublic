...
def _chunk_slice(
    ...
    batch_dims = t.shape[:num_batch_dims]
    ...
    return torch.cat([s.view((-1,) + t.shape[num_batch_dims:]) for s in sliced_tensors])
...
def chunk_layer(
    ...
    initial_dims = [shape[:num_batch_dims] for shape in _fetch_dims(inputs)]
    orig_batch_dims = tuple([max(s) for s in zip(*initial_dims)])

    def _prep_inputs(t):
        if not low_mem:
            if not sum(t.shape[:num_batch_dims]) == num_batch_dims:
                t = t.expand(orig_batch_dims + t.shape[num_batch_dims:])
            t = t.reshape(-1, *t.shape[num_batch_dims:])
        else:
            t = t.expand(orig_batch_dims + t.shape[num_batch_dims:])
        return t
    ...
    if _out is not None:
        reshape_fn = lambda t: t.view([-1] + list(t.shape[num_batch_dims:]))
