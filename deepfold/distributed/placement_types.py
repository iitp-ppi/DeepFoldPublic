...
from deepfold.distributed._comm import all_gather_tensor, mesh_broadcast, mesh_scatter, reduce_scatter_tensor
...
class Shard(Placement):
    ...
    def _split_tensor(
        ...
        *,
        ...
        """
        This function uses `torch.chunk` to split a tensor into `num_chunks` shards along the
        Shard placement dimension, and return a list of shards with their pad sizes.
        """
        ...
        # Chunk tensor over dimension with padding if necessary
    ...
    def _local_shard_size_on_dim(
        ...
        """
        Returns the local shard size and offset on a given dimension.
        """
        assert (
    ...
    def _reduce_shard_tensor(
        ...
        tensor: torch.Tensor,
        mesh: DeviceMesh,
        reduce_op: cc.ReduceOp.RedOpType,
        mesh_dim: int,
    ) -> torch.Tensor:
        """
        ...
        output = reduce_scatter_tensor(tensor, reduce_op.name, scatter_dim=self.dim, group=(mesh, mesh_dim))
...
class Replicate(Placement):
    ...
    def _replicate_tensor(
        ...
        the first coordinate on the mesh dimension as source).
        """
        my_coordinate = mesh.get_coordinate()
        if my_coordinate is None:
            # if rank is not part of mesh, we simply return an empty tensor
...
class _Partial(Placement):
    def __init__(self, reduce_op: cc.ReduceOp.RedOpType = cc.ReduceOp.SUM):
        self.reduce_op: cc.ReduceOp.RedOpType = reduce_op

    def _to_replicate(self, tensor: torch.Tensor, mesh: DeviceMesh, mesh_dim: int) -> torch.Tensor:
        return cc.all_reduce(tensor, reduceOp=self.reduce_op.name, group=(mesh, mesh_dim))
...
# Used to propagate the placements
@dataclass
class DTensorSpec:
    mesh: DeviceMesh
    ...
    def __eq__(self, other: object) -> bool:
        if not (isinstance(other, DTensorSpec) and self.mesh == other.mesh and self.placements == other.placements):
            return False
    ...
    def from_dim_map(
        ...
        Args:
            ...
            dim_map: (ist[int]
                A list of integer that represents sharding on each
                tensor dimension, see `dim_map` property doc for details
            sums: List[int])
                a list of integer that represents the dist tensor have
                pending sum on which device mesh dimension.
            tensor meta: TensorMeta
                DTensor metadata

        Returns:
            A `DTensorSpec` object.
        """
    ...
    def is_replicated(self):
        """
        Return `True` if the current `DTensorSpec` replicates on all mesh dims
        """
