...
from deepfold.distributed import _backend as cc
...
class MeshEnv:
    ...
    def num_hosts(device_type: str) -> int:
        return cc.get_world_size() // MeshEnv.num_devices_per_host(device_type)
...
class DeviceMesh:
    ...
    def _get_or_create_default_group(self):
        default_initialized = cc.is_initialized()
        if not default_initialized:
            cc.init_process_group()

        world_size = cc.get_world_size()
    ...
    def _init_process_group(self, _validate_mesh):
        ...
        if self.mesh.ndim == 1 and self.mesh.numel() == cc.get_world_size():
            # If the mesh is the same as world_pg, just append the default pg to the first dim groups
            dim_group_infos.append(cc._get_group_tag(cc._get_default_group()), list(range(cc.get_world_size())))
        else:
            # Create sub-pgs
            for dim in range(self.mesh.ndim):
                ...
                for dim_mesh in pg_ranks_by_dim:
                    ...
                    dim_group = cc.new_group(ranks=subgroup_ranks)
                    ...
                    if self.get_rank() in subgroup_ranks:
                        ...
                        dim_group_infos.append((cc._get_group_tag(dim_group), subgroup_ranks))
    ...
    def get_dim_groups(self, mesh_dim: Optional[int] = None) -> Union[cc.ProcessGroup, List[cc.ProcessGroup]]:
        if not hasattr(self, "_dim_group_infos"):
            raise RuntimeError("DeviceMesh process groups not initialized")
        if mesh_dim is not None:
            return cc._find_pg_by_ranks_and_tag(*self._dim_group_infos[mesh_dim])
        else:
            dim_groups = []
            for mesh_dim in range(self.mesh.ndim):
                dim_groups.append(cc._find_pg_by_ranks_and_tag(*self._dim_group_infos[mesh_dim]))
            return dim_groups
    ...
    def _validate_mesh(self):
        ...
        mesh_tensor = _comm.all_gather_tensor(self_mesh, gather_dim=0, group=cc._get_default_group())
        mesh_tensor_chunked = torch.chunk(mesh_tensor, cc.get_world_size())
        for other_rank, other_mesh in enumerate(mesh_tensor_chunked):
            if not torch.equal(self_mesh, other_mesh):
                raise RuntimeError(
                    ...
                    f"rank {cc.get_rank()} has mesh {self_mesh} while rank {other_rank} "
